...
import { AgentStream, ChatMemoryBuffer, ChatMessage } from "llamaindex";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { memoryStore, monologue } = await CognitionStream({llm, embedder, messages, userMessage})
    
    //Main character enters the scene:
    const agent = await createBlackCatEngine(memoryStore, userMessage.content, monologue);
