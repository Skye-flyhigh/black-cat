...
import { innerMonologue } from '../reflection/MonologueGenerator';
import { SystemInjector, InjectionContext } from '../injection/SystemInjector';
...
export class CognitionDaemon {
  ...
  private systemInjector: SystemInjector;
  ...
  private async generateSystemInjections(context: MemoryContext, monologue: string): Promise<void> {
    console.log("🧠 [CognitionDaemon] Generating system injections...");
    
    // Use our new SystemInjector for LLM-powered prompt generation
    const injectionContext: InjectionContext = {
      state: this.state,
      memoryContext: context,
      recentObservations: this.observationQueue.map(obs => obs.utterance),
      monologue
    };
    
    // Try LLM-powered injection first
    const llmInjection = await this.systemInjector.generateSystemInjection(injectionContext);
    if (llmInjection) {
      this.injectionQueue.push(llmInjection);
      console.log("🧠 [CognitionDaemon] Added LLM-generated injection");
    }
    
    // Fallback to energy-based injection if LLM didn't generate anything
    if (!llmInjection) {
      const energyInjection = await this.systemInjector.generateEnergyBasedInjection(this.state);
      if (energyInjection) {
        this.injectionQueue.push(energyInjection);
        console.log("🧠 [CognitionDaemon] Added energy-based injection");
      }
    }
